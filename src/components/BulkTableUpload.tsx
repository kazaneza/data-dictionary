...
interface BulkTableUploadProps {
  selectedSourceId: string;
  selectedDatabaseId: string;
  onSuccess: () => void;
}

export default function BulkTableUpload({ 
  selectedSourceId, 
  selectedDatabaseId,
  onSuccess 
}: BulkTableUploadProps) {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      setLoading(true);
      const tables = await api.parseTablesFile(file);
      const createdTables = await api.createTablesBulk(selectedDatabaseId, tables);
      toast.success(`Successfully created ${createdTables.length} tables`);
      onSuccess();
    } catch (error) {
      ...
      >
        ...
        <p className="text-lg mb-2">
          {loading ? 'Uploading...' : selectedDatabaseId
            ? 'Drag & drop your tables file here, or click to select'
...
{`[
  {
    ...
    "category_id": "optional-uuid"
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <pre className="text-sm overflow-x-auto">
{`Table Name,Description,Category ID
customers,Customer data,optional-uuid
orders,Order information,optional-uuid`}
            </pre>
          </div>
        </div>
      </div>
